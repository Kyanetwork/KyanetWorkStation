...
- 启动 preflight、MeowStatus 关闭门与三态代码路径、临时备份恢复以及至少
  一个可观测可重试通知路径已有自动或可审计证据；外部状态响应的严格边界
  仍由 D-007 跟进。
...
- [ ] 测试关闭、超时/不可达和成功三种状态，包含响应大小/字段规范化边界。
      当前自动冒烟覆盖 `disabled`；外部响应边界归入 D-007，待后续测试与部署验证。
...
- [ ] 若跨驱动原子 outbox 在本任务中无法安全完成，则实现从业务记录到
      provider 的受控手动重试命令，并在发布门禁明确列为降级边界；不得保留
      无持久状态的 fire-and-forget。当前 outbox 入队异常仍记录日志并需要
      R-004 的人工补偿设计。
