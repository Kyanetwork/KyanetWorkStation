...
| P2 项目管理基础能力 | 已实现/P2 | `project`/`project_milestone`/`project_item` 三表增量模型；Feedback 与 WorkTask 分立归属、里程碑生命周期、自动/自定义完成度、管理员 hash 详情和按项目/关系开关控制的公共项目列表/详情；公共工作项仅返回安全摘要，项目容器与公共投影边界已稳定 |
| P2 Kanban 基础能力 | 已实现/P2 | 管理员项目详情提供关系视图/Kanban 切换；Feedback 与 WorkTask 使用独立原生状态分区，状态保存经 `POST /api/admin/project/item/status`、项目范围校验和 `project.item.status` 审计；公共端仅支持独立工作项摘要，不开放 Kanban |
| Kanban 高级能力与公共 Kanban（子任务、依赖、拖拽和批量操作） | P2 计划 | 建立在项目数据契约之上，需另行设计关系图、并发、批量交互和公共隐私投影；当前基础 Kanban 不扩展这些能力 |
| 用户侧/运维侧 AI | P2 计划 | 依赖 Provider、脱敏和权限边界；当前不开放 |
| 新 KyanetAccount 联动 | 暂缓/独立计划 | 当前不推进；未来按新协议和重构后的 Account 边界重新设计，不自动认领历史匿名记录 |
