...
- 外部服务响应必须有超时、响应大小上限和字段规范化；当前 MeowStatus
  响应大小及 favicon MIME/解码边界仍由 D-007 补齐，不能视为已完成。
- 外部服务的详细错误只供日志/管理员诊断，公共页面展示短错误说明。
- 未来增加个人项目服务时，优先实现一个小适配器和稳定的卡片 DTO，不改变主业务表。

## 通知边界

通知是提交后的旁路能力，不应决定反馈或 WorkTask 是否写入成功。当前 `notification_delivery` 记录事件 ID、目标标签、尝试次数、最后错误和可重试状态；在持久化投递完成前，不宣称“已可靠送达”。
