...
`publicKey`。基础信息、里程碑、更新时间、完成度和工作项分别受项目级开关控制；工作项还必须通过关系级
`public_visible` 开关，仅以来源类型、标题、原生状态、可选公开回复和更新时间投影。公共投影只包含名称、
说明、有效里程碑、安全日期/完成度和上述工作项摘要，不包含来源 ID、项目/里程碑关系 ID、Kanban 分区、
正文、联系方式、管理员字段或数据库内部 ID。
