...
- 使用 Node.js 24.x LTS；当前锁文件目标为 `better-sqlite3 ^13.0.3`、
  `nodemailer ^9.1.1`，并确保原生模块在实际运行时加载成功。不要用旧的 12.x
  锁文件替换当前发布基线。
- 应用默认绑定回环地址，由 Nginx、IIS 或 Caddy 处理公网 TLS。
- `.env`、数据库、备份、日志和真实域名配置只存在于部署环境。
- 生产部署前必须完成[发布门禁](../testing/release-checklist.md)。

本文命令中的 `127.0.0.1:3000` 是应用默认端口；如果生产 `.env` 设置了其他
`PORT`，所有 health、监听和代理检查都必须替换为该实际端口。端口只应在本机回环
或已确认的私网边界监听，公网访问仍通过反向代理。
