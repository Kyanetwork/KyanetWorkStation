# Workstation 路线图

路线图只描述已确认的未来工作；“计划”不等于“已实现”。每个实现阶段应建立独立 Trellis 任务并重新检查范围。

## 阶段总览

```text
P0 可靠性/隐私/安全/运行时基线
  ↓ 发布门禁通过
P1 统一首页 + 工作收件箱 + 我的工作区 + 服务卡片
  ↓ 工作台数据契约稳定
P1 管理员 AI Copilot + 个人知识助手
  ↓ AI 安全和审计稳定
P2 项目管理基础 + Kanban 基础能力（当前已实现）
  ↓ 项目数据契约稳定
P2 Kanban 高级能力 + 用户侧/运维侧 AI
  ↘ 新 KyanetAccount 联动（暂缓，独立设计）
```

## P0：可靠性与边界基线

### P0-1 `p0-remove-legacy-account`（本任务已执行）

移除旧 KyanetAccount 路由、中间件、配置和活动依赖；历史 schema/数据保留，字段/表的最终删除仍需独立迁移任务、备份和回滚演练。不自动认领历史匿名记录。

### P0-2 `p0-privacy-security-boundary`（代码边界已执行）

修复公共 highlights 和历史 Account 列表的 DTO 泄露，补齐直连伪造转发头回归、WorkTask 安排中清空负责人/计划时间语义，记录未来 Account state/nonce 要求，并补回归测试。D-007 的 MeowStatus 外部响应和 favicon 严格大小/MIME 边界已完成本地实现；D-004 的真实反向代理边界已在当前云服务器发布中验证，拓扑变化时按发布模板重演。

### P0-3 `p0-runtime-test-baseline`（本任务已执行）

锁定支持的 Node 版本，解决 `better-sqlite3` ABI，修复临时数据库/端口/子进程清理，建立 API 冒烟、依赖审计和跨数据库验证矩阵。

### P0-4 `p0-operations-reliability`（代码基础能力已执行，发布证据待补）

增加配置启动自检、MeowStatus 显式启用策略、隔离 SQLite 备份恢复自动演练、通知持久化投递状态/人工重试、outbox 入队失败 handoff、日志轮转和统一发布门禁。本机真实备份/通知链路与当前云服务器代理/TLS 门禁已有记录；若发布目标或 provider 变更，必须按模板重演对应证据。

### P0 退出条件

- 支持的 Node 版本可启动，`npm test` 全部通过或每个阻塞有明确豁免记录。
- 公共和用户 DTO 通过敏感字段回归测试。
- 备份在临时环境成功恢复，至少一条通知链路可验证。
- 依赖审计有升级或风险接受结论。
- 发布、观测和回滚证据完整。

## P1：可用的工作台

### P1-1 `p1-workspace-inbox`（已实现）

保留反馈/WorkTask 独立表，通过统一读取层展示待处理事项、状态、优先级、负责人、计划时间、标签、关键词筛选和最近更新。

### P1-2 `p1-my-workspace`

提供安全的“我的提交/任务”视图，展示状态、公开回复、时间和必要摘要；不得暴露管理员备注、联系方式、账号快照或其他用户数据。当前匿名模式不提供历史查询，账号视图待未来 Account 重构。

### P1-3 `p1-service-cards`（已实现）

将 MeowStatus 包装为独立服务卡片，显示可用性、Minecraft 状态和更新时间，支持失败/关闭状态和快捷入口；未来按相同边界添加其他个人项目服务。

### P1 工作台当前状态

- 统一首页、管理员工作收件箱和 MeowStatus 服务卡片已落地并有回归覆盖。
- `p1-my-workspace` 的安全历史视图仍待未来 Account 重构后单独设计；匿名模式暂不提供历史查询。
- 工作台后续退出条件仍包括稳定 DTO、筛选排序和服务卡片故障隔离，发布时按门禁持续验证。

## P1：管理员 AI Copilot

### P1-4 `p1-admin-ai-copilot`（已实现，当前阶段完成 P1-A 增强）

采用轻量 Provider Adapter，默认关闭；已实现摘要、分类、优先级/标签建议、相似条目提示
和回复草稿。支持多个命名 profile 与单 active 热切换，输入脱敏，输出仅为短期建议，状态
修改、删除和通知发送必须人工确认。profile 还支持 Responses `reasoning_effort`（low/
medium/high/xhigh/max）和最多 2000 个 Unicode 字符的受控工作指令。详见 [AI 计划](ai-assistant.md)。

### P1-5 `p1-admin-knowledge-assistant`（已实现）

管理员知识助手读取部署环境配置的外部 Markdown/TXT 目录，显式建立版本化索引并进行
确定性检索。回答返回服务端映射的引用和 `document/mixed/general` 依据；无充分命中时
标注“非文档依据/需核验”。问答历史进入三数据库兼容表，默认保留 30 天，自动清理可在
管理员页面开关（启动一次 + 每小时执行），手动清理和单条删除始终可用。

知识助手不接受浏览器路径、不监听原始目录、不执行文档指令、不读取未索引文件，也不修改
反馈/WorkTask 或发送通知。生产发布顺序是：备份 → 同步/挂载只读目录 → 配置 `.env` →
重启 → `npm run reindex-knowledge` → 管理员页面验证。Provider 诊断、指标和 prompt 契约
收敛已在 P1-B 完成；prompt 对比、批量摘要等仍不提前建设自动 fallback 或智能路由。

### P1-6 `r002-server-export-audit`（已实现/待发布验证）

将管理员反馈和 WorkTask 导出改为服务端固定 250 行分批 CSV 流，使用
`ADMIN_EXPORT_MAX_ROWS`（默认 10000，范围 100–100000）保护资源，不在浏览器收集全量结果。
同时为状态、删除、主页展示、备注/回复、安排/创建、AI 决策、通知重试和状态设置等动作
写入三数据库兼容的 `admin_audit`，只保留白名单脱敏元数据，并提供管理员分页查询 API。
审计随现有备份保留；真实生产下载、超限提示、代理链路和 MySQL/PostgreSQL 实链路仍按发布
门禁验证，暂不建设审计前端页面。

### P1-B `p1b-provider-diagnostics-metrics`（已实现/待真实 Provider 验证）

为管理员提供按 profile 的显式真实诊断和 AI 请求聚合指标。诊断只发送固定 sentinel 探针，
可验证 Chat、Responses、Anthropic 三种协议的 HTTP/JSON/文本/响应大小边界，不切换 active
profile，也不创建建议或知识历史；Responses profile 的 `reasoning_effort` 仅记录是否随请求
发送，不将一次探针解释为完整能力证明。指标记录 Copilot、知识问答和诊断的成功/失败/超时、
耗时、已知 token 和稳定错误码，三数据库等价建表，默认保留 30 天并受环境开关控制；汇总
接口限制为 1–720 小时和最多 100 个 operation/protocol 分组。

本地 stub 回归、API/UI 脱敏和三数据库 schema 静态覆盖已完成；发布前仍需在受控环境使用真实
Provider 点击一次诊断并记录脱敏结果。该任务不引入自动 fallback、智能路由、并行多 Provider、
向量数据库或外部监控平台。

## P2：扩展能力

### P2-1 `p2-project-management`（基础能力已实现）

当前版本提供轻量项目容器：项目创建、编辑、软归档/恢复；Feedback 与 WorkTask 分立归属、单条
绑定/换里程碑/解绑；里程碑排序、完成、撤销/恢复；自动或自定义完成度；管理员 hash 详情页。
公共端通过随机不可变 `publicKey` 提供项目列表和详情，基础信息、里程碑、更新时间、完成度和工作项分别
由独立开关控制；工作项还需逐条公开，仅以来源类型、标题、原生状态、可选公开回复和更新时间投影，
不返回来源正文、联系方式、管理员字段或内部 ID。三数据库 schema、
孤儿关系清理、审计和回归测试已加入当前实现。

项目容器基础能力已完成；Kanban 基础视图另见 P2-1b。子任务、依赖关系、批量归属、拖拽、甘特图和
公共 Kanban 不属于当前基础范围，需在项目数据契约稳定后另立 Trellis 任务设计。

### P2-1b `p2-kanban-foundation`（已实现）

管理员项目详情提供关系视图与 Kanban 切换，默认仍进入关系视图。Kanban 复用安全项目详情 DTO，按
Feedback/WorkTask 划分两个独立分区，并使用各自原生状态列；卡片在列内按最近更新时间和项目关系 ID
稳定排序。状态通过 `POST /api/admin/project/item/status` 受控保存，服务端校验项目 active、来源归属和
来源专属状态，实际变化同步项目更新时间并写入脱敏审计；相同状态保存不重复更新时间。公共项目列表仍只
返回基础信息，公共详情可按项目总开关和关系级开关返回两个独立的工作项摘要分区，但不返回 Kanban 数据；
关系视图继续负责里程碑、绑定/解绑和逐条公开设置。

本阶段刻意不加入手动排序、拖拽、批量操作、子任务、依赖、实时协作或公共 Kanban，以保持个人使用场景的
低维护成本和现有技术架构稳定。

### P2-2 `p2-account-and-ai-expansion`（暂缓，独立计划）

KyanetAccount 当前开发停滞，不参与项目基础或 Kanban 主线。未来需要时再独立设计新的 Account 联动、
用户侧提交助手、面向更广泛运维场景的 AI 和公众账号模式；当前管理员知识助手不自动扩展为用户侧或运维
写操作，历史匿名记录不自动归属新账号。

## 暂缓清单

原生文件存储、实时聊天、计费、多租户、复杂 RBAC、通用插件市场和与个人项目的全面自动化写操作暂不进入主线。
